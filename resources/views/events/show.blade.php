<x-app-layout title="Detail Kegiatan - {{ $event->title }}">
    <div class="max-w-5xl mx-auto">
        {{-- Breadcrumb --}}
        <nav class="flex text-sm text-gray-500 mb-4">
            <a href="{{ route('dashboard') }}" class="hover:text-green-600">Beranda</a>
            <span class="mx-2">/</span>
            <span class="text-gray-800 font-medium">Detail Kegiatan</span>
        </nav>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {{-- LEFT --}}
            <div class="lg:col-span-2 space-y-6">
                {{-- Banner --}}
                <img
                    src="{{ $event->banner_path
                        ? asset('storage/' . $event->banner_path)
                        : 'https://images.unsplash.com/photo-1593113598332-cd288d649433?auto=format&fit=crop&w=1200&q=80' }}"
                    class="w-full h-80 object-cover rounded-3xl shadow-sm"
                    alt="{{ $event->title }}"
                >

                {{-- Main content --}}
                <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                    <div class="flex items-center gap-3 mb-4">
                        <span class="px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full uppercase tracking-wider">
                            {{ ucfirst($event->status) }}
                        </span>
                        <span class="text-gray-400 text-sm">•</span>
                        <span class="px-3 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full uppercase tracking-wider">{{ $event->category }}</span>
                    </div>

                    <h1 class="text-3xl font-bold text-gray-800 mb-6">
                        {{ $event->title }}
                    </h1>

                    <h2 class="font-bold text-gray-800 mb-2">Deskripsi Kegiatan</h2>
                    <p class="text-gray-600 leading-relaxed whitespace-pre-line">
                        {{ $event->description }}
                    </p>
                </div>
            </div>

            {{-- RIGHT --}}
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm sticky top-24">
                    {{-- Info --}}
                    <div class="space-y-4 mb-6">
                        {{-- Date --}}
                        <div class="flex items-start gap-3">
                            <div class="p-2 bg-green-50 rounded-lg">📅</div>
                            <div>
                                <p class="text-xs text-gray-500">Tanggal & Waktu</p>
                                <p class="text-sm font-semibold">
                                    {{ $event->start_date->format('d M Y, H:i') }}
                                    –
                                    {{ $event->end_date->format('d M Y, H:i') }}
                                </p>
                            </div>
                        </div>

                        {{-- Location --}}
                        <div class="flex items-start gap-3">
                            <div class="p-2 bg-green-50 rounded-lg">📍</div>
                            <div>
                                <p class="text-xs text-gray-500">Lokasi</p>
                                <p class="text-sm font-semibold">
                                    {{ $event->location_name }}
                                </p>
                            </div>
                        </div>

                        {{-- Volunteers --}}
                        <div class="flex items-start gap-3">
                            <div class="p-2 bg-green-50 rounded-lg">👥</div>
                            <div>
                                <p class="text-xs text-gray-500">Relawan</p>
                                <p class="text-sm font-semibold">
                                    {{ $event->participants->count() }}
                                    /
                                    {{ $event->target_volunteers ?? '∞' }}
                                </p>
                            </div>
                        </div>
                    </div>

                    {{-- Organizer --}}
                    <div class="bg-gray-50 p-4 rounded-2xl mb-6">
                        <p class="text-[10px] uppercase font-bold text-gray-400 mb-2">
                            Penyelenggara
                        </p>
                        <div class="flex items-center gap-3">
                            <img
                                src="https://ui-avatars.com/api/?name={{ urlencode($event->organizer->name) }}"
                                class="w-10 h-10 rounded-full"
                                alt="{{ $event->organizer->name }}"
                            >
                            <div>
                                <p class="text-sm font-bold text-gray-800">
                                    {{ $event->organizer->name }}
                                </p>
                                <p class="text-xs text-yellow-500 font-bold">
                                    ⭐⭐⭐⭐⭐ ({{ $event->organizer->rating }})
                                </p>
                            </div>
                        </div>
                    </div>

                    {{-- Actions --}}
                    <button
                        class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-2xl transition shadow-lg shadow-green-100 mb-3">
                        Ikut Kegiatan
                    </button>

                    @if($event->canBeFinished())
                        <button
                            class="w-full border-2 border-green-500 text-green-600 font-bold py-3 rounded-2xl hover:bg-green-50 transition">
                            Selesaikan Kegiatan
                        </button>
                    @endif
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
